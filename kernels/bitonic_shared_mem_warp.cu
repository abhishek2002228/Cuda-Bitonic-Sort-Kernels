...
//we also use warp shuffle if the comparision distance is < warp size
__device__ __forceinline__ int bitonic_select(int ai, int aj, bool up, bool lower_index) {
    int min_val = (ai < aj) ? ai : aj;
    int max_val = (ai > aj) ? ai : aj;
    return up ? lower_index ? min_val : max_val : lower_index ? max_val : min_val;
}
...
__global__ void bitonic_sort_shared(int *a, int n, bool finalUp) {
    ...
    if (global_tid < n) {
        ...
        shared[local_tid] = 1000; // pad with max value
    }
    ...
    for (unsigned int s=2; s <= blockDim.x; s <<= 1) {
        ...
        // Now d < 32, use warp shuffle
        int my_val = shared[local_tid];
        unsigned int lane = local_tid & 31; // lane index within the warp
        for (; d > 0; d >>= 1) {
            int partner_val = __shfl_xor_sync(0xFFFFFFFF, my_val, d);
            bool up = ((global_tid & s) == 0);
            if (!finalUp) up = !up;
            bool is_lower_index = ((lane & d) == 0);
            my_val = bitonic_select(my_val, partner_val, up, is_lower_index);
        }
        //update my value in shared memory
        shared[local_tid] = my_val;
        __syncthreads(); // ensure all threads have completed this stage
    }
...
__global__ void bitonic_sort_shared_fused(int *a, int n, int s, int d, bool finalUp) {
    ...
    //__syncthreads(); //TODO: remove and check 
    // write sorted data back to global memory
    if (global_tid < n) {
        a[global_tid] = shared[local_tid];
    }
