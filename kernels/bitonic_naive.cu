...
__global__ void bitonic_sort_kernel(int *a, int n, int s, int d, bool finalUp) {
...
void bitonic_sort(int *d_arr, int size, bool finalUp = true) {
    ...
    for (int s = 2; s <= size; s <<= 1) {
        for (int d = s >> 1; d > 0; d >>= 1) {
            bitonic_sort_kernel<<<blocksPerGrid, threadsPerBlock>>>(d_arr, size, s, d, finalUp);
...
if (new_size > size)
    bitonic_sort(d_arr, new_size);
else
    bitonic_sort(d_arr, size);
